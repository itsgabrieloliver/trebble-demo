...
<div class="body">
	{#if spotifyParam === 'connected'}
		<p class="ok-note">Spotify connected. Your library and playlists are live.</p>
		...
		<section class="hero">
			...
			<div class="hero-copy">
				...
				<p class="meta hero-note">
					Pulled from your Spotify top tracks. Playback picks full tracks on Premium and 30 second
					previews otherwise.
				</p>
				<Button onclick={() => play(0)}>Play this track</Button>
			</div>
		</section>
	{/if}
	...
	<section class="stats">
		...
		/>
		<StatTile label="Shares posted" value={data.shareTotal} />
	...
	<section class="section">
		<div class="section-head">
			<div>
				...
				<p>The latest shares from everyone here</p>
			</div>
				...
				{#each data.recentShares as share (share.id)}
					<li>
						...
						<div class="recent-copy">
							<p class="recent-line">
								<strong>{share.displayName}</strong> shared
		...
		{:else}
			<EmptyState
				...
				description="Share a track from your library and it shows up here for everyone, with reactions and comments underneath."
				icon="◎"
			>
				{#snippet action()}
